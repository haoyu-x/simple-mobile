        ...
        for episode_idx, episode_dir in enumerate(tqdm(episode_dirs)):
            ...
            actions = [
                np.concatenate((
                    ...
                    action["arm_left"]['arm_pos'],
                    Rotation.from_quat(action["arm_left"]['arm_quat']).as_rotvec(),  # Convert quat to axis-angle
                    action["arm_left"]['gripper_pos'],
                    
                    action["arm_right"]['arm_pos'],     
                    Rotation.from_quat(action["arm_right"]['arm_quat']).as_rotvec(),  # Convert quat to axis-angle
                    action["arm_right"]['gripper_pos'],
...
# Observation key: arm_pos
# Observation key: arm_quat
# Observation key: gripper_pos
# Observation key: base_image
# Observation key: wrist_image
# action dim: 10 (base_velocity(3), arm_pos(3), arm_quat(3), gripper_pos(1))
